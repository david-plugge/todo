...
- Access Token: 15 Minuten. Refresh Token: maximal 30 Tage ab Erteilung, mit Rotation. Wiederverwendung verbrauchter Codes oder Refresh Tokens widerruft die gesamte Grant-Familie. Refresh widerruft auch vorherige Access Tokens.
...
Das Container-Deployment aktiviert PocketBases IP-basierten Rate-Limiter einmalig per Migration. Methodenspezifische Regeln schützen insbesondere Registrierung, Authorization, Token-Austausch und MCP, ohne normale Sync-Bursts zu drosseln. Ein täglicher transaktionaler Cron entfernt abgelaufene Zustimmungsanfragen und vollständig inaktive Tokenfamilien. Verbrauchte Refresh-Tokens einer noch aktiven Familie bleiben für die Replay-Erkennung erhalten; dynamisch registrierte Clients und die Todo-Sync-Historie werden nicht bereinigt. Proxy-, Betriebs- und Logdetails stehen in [deployment.md](deployment.md).
